...
- AutoAim now arms only when the alliance goal tag is **detected** and a finite per-fiducial heading exists; rotation output still zeros when tx is invalid, but telemetry and the Tag Visible line reflect detection even when aim is gated off (applies to both TeleOp toggle and AutoAimSpeed assists in Auto).
